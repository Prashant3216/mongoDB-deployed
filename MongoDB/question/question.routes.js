...
app.post("/", async (req, res) => {
  ...
  // console.log(body)
  try {
    await qesModel.insertMany(body);
...
app.get("/joblist", async (req, res) => {
  try {
    let products = await qesModel.find();
...
app.get("/joblist/filter", async (req, res) => {
  let { role } = req.query;
  try {
    let products = await qesModel.find({ role: role });
    return res.status(200).send(products);
  } catch (error) {
    return res.status(404).send("Something went wrong");
  }
...
app.get("/joblist/sort/latest_to_oldest", async (req, res) => {
  try {
    let products = await qesModel.find().sort({ postedAt: -1 });
    return res.status(200).send(products);
  } catch (error) {
    return res.status(404).send("Something went wrong");
  }
...
app.get("/joblist/search", async (req, res) => {
  ...
  try {
    let products = await qesModel.find({ language: language });
    return res.status(200).send(products);
  } catch (error) {
    return res.status(404).send("Something went wrong");
  }
